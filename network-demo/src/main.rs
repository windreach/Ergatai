...
use agent_network::AgentNetwork;
...
async fn main() {
    ...
    transport.join_channel(&"bob".to_string(), "code_review").await.unwrap();
    transport.join_channel(&"charlie".to_string(), "code_review").await.unwrap();
    ...
    println!("  详细信息: {}", serde_json::to_string_pretty(&health.details).unwrap());
