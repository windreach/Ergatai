...
### ✅ 1. DAG 执行的 HTTP ACP 集成

**文件**: `crates/ergatai-collab/src/agent_launcher.rs`

**实现内容**:
- ✅ 从 AgentRegistry 获取代理的 ACP 端点
- ✅ 通过 HttpConnectionManager 建立 HTTP 连接
- ✅ 发送指令给代理
- ✅ 更新代理状态
- ✅ 支持 DAG 节点关联

**代码位置**: `spawn_acp_session` 函数 (行 423-540)

**关键代码**:
```rust
async fn spawn_acp_session(&self, agent_id: &str, ...) -> ErgataiResult<()> {
    // 1. 获取 ACP 端点
    let acp_endpoint = registry.get_acp_endpoint(agent_id).await?;
    
    // 2. 建立 HTTP 连接
    let session_id = http_manager.connect(agent_id, &acp_endpoint, cwd, kind).await?;
...
**文件**: `crates/ergatai-acp/src/agent_registry.rs`
...
[TEST] ACP endpoint registered!
[TEST] ACP endpoint works!
...
- 原因: simple-agent 只实现了简单的 HTTP API，未实现完整的 ACP 协议
- 影响: 不影响核心架构，只是示例代理需要完善

**解决方案**: 
- 在 simple-agent 中实现完整的 ACP 协议端点
...
- `agent_launcher.rs` spawn_acp_session: ~120 行

### 修改文件
- `ergatai-acp/src/lib.rs`: 添加 agent_registry 模块
- `ergatai-acp/src/http_client.rs`: 添加全局访问器
...
- `ergatai-collab/src/agent_launcher.rs`: 实现 spawn_acp_session
...
1. **核心功能完成**: DAG 执行可以通过 HTTP ACP 工作
2. **架构清晰**: 全局单例模式，模块间解耦
3. **测试通过**: 集成测试验证了基本流程
4. **代码质量**: 编译成功，错误处理完善
