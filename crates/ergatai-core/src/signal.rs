...
/// 1. File access control (release locks, stop watchdogs)
/// 2. NATS (last — other shutdowns may publish completion events)
///
...
async fn graceful_shutdown() -> ErgataiResult<()> {
    ...
    tracing::info!("Step 1/2: shutting down file access control...");
    ...
    // 2. NATS (last — other steps may publish completion events)
    tracing::info!("Step 2/2: shutting down NATS...");
    match tokio::time::timeout(STEP_TIMEOUT, async {
