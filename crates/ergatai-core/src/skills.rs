...
fn extract_description(content: &str) -> String {
    ...
    if content.starts_with("---") {
        if let Some(end) = content.find("\n---") {
            // Safety check: ensure we have enough bytes to skip the opening "---\n"
            if end < 4 {
                // Malformed frontmatter (too short), fall through to content parsing
                return content
                    .lines()
...
pub fn scan_skills() -> ErgataiResult<Vec<SkillInfo>> {
...
pub fn get_skill_detail(name: String) -> ErgataiResult<SkillDetail> {
    ...
    Err(ErgataiError::internal(format!("Skill not found: {}", name)))
}
