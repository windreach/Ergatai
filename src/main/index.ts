...
// Linux: disable sandbox (chrome-sandbox SUID not set in dev)
...
// ACP replaces Claude/Codex engines — stub these functions
const getAllMcpConfigHandler = async () => []
const hasActiveClaudeSessions = () => false
const abortAllClaudeSessions = () => {}
const getAllCodexMcpConfigHandler = async () => []
const hasActiveCodexStreams = () => false
const abortAllCodexStreams = () => {}
import {
  createMainWindow,
  createWindow,
  getWindow,
...
<head>
  ...
  <title>Ergatai - Authentication</title>
  <style>
...
<head>
  <meta charset="UTF-8">
  <link rel="icon" type="image/svg+xml" href="${FAVICON_DATA_URI}">
  <title>Ergatai - MCP Authentication</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    :root {
      --bg: #09090b;
...
if (gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    if (process.platform === "win32") {
      app.setAppUserModelId(IS_DEV ? "com.ergatai.app.dev" : "com.ergatai.app")
    }

    console.log(`[App] Starting Ergatai${IS_DEV ? " (DEV)" : ""}...`)
                ...
                if (isCliInstalled()) {
                  ...
                  if (result.success) {
                    dialog.showMessageBox({
                      ...
                      detail:
                        "You can now use '1code .' in any terminal to open Ergatai in that directory.",
                    })
                    buildMenu()
                  } else {
